...
extension VoiceRecorderViewModel {
    func recordBtnTapped() {
        selectedRecordedFile = nil // 다시 재생할꺼니까 선택된 것이 없도록 초기화
        
        if isPlaying {
            stopPlaying()
            startRecording()
        } else if isRecording { // 녹음 중인데 다시 눌렀을 때
            stopRecording()
        } else {
            startRecording()
        }
    ...
    private func startRecording() {
        ...
        let settings = [
            ...
            AVSampleRateKey: 12000, // 샘플링 비율 설정
        ...
        do {
            ...
            displayAlert(message: "음성메모 녹음 중 오류 중 발생했습니다.")
        }
    }
...
extension VoiceRecorderViewModel {
    ...
    // AVAudioPlayerDelegate에서 제공하는 메서드
